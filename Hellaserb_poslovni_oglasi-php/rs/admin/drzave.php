	...
	<nav class="navbar navbar-inverse navbar-static-top" id="my-navbar">
		...
		<div class="row">
			...
			<?php
				...
				if ($akcija !== false)
				{
					switch ($akcija)
					{
						...
						$naziv_drzave_srb=$_POST['naziv_drzave_srb'];
						$naziv_drzave_gre=$_POST['naziv_drzave_gre'];

						if (!empty($naziv_drzave_srb) and !empty($naziv_drzave_gre))
						{
							mysql_query("INSERT INTO drzave(naziv_drzave_srb, naziv_drzave_gre) VALUES ('{$naziv_drzave_srb}', '{$naziv_drzave_gre}')");
						}
						...
						mysql_query("DELETE FROM drzave WHERE id_drzava=$id");
			...
			<?php if ($prikazi_listu) {
				$result = mysql_query("SELECT * FROM drzave order by naziv_drzave_srb, naziv_drzave_gre");
			?>

				<h3>Dodaj novu državu</h3>
				<form id="contactform" action="drzave.php?akcija=dodaj" method="post">
					<input type="text" name="naziv_drzave_srb" placeholder="Naziv države srb..." />
					<input type="text" name="naziv_drzave_gre" placeholder="Naziv države gre..." />
				...
				<table class="table table-bordered">
					...
					<?php while ($row = mysql_fetch_assoc($result)) { ?>
						<tr>
							...
							<td>
								<a href="drzave.php?akcija=obrisi&id=<?php echo $row['id_drzava']; ?>">Obrisi</a>
							</td>
						</tr>
					<?php } ?>	
					</tbody>
