...
var novi='0';
    ...
    $('#posalji').click(function() {
        ...
        $.ajax({
            ...
            success: function (msg) {
                ...
                if (msg == 'uspeh') {
                    $('#logoglas').hide();
                    $("#greska").html('Uspešno ste se ulogovali! <button class="btn btn-theme " style="border:2px solid white;" type="button" id="postavi" name="postavi">Postavi oglas</button>');
    ...
    $("#posalji2").click(function() {
        ...
        $.ajax({
            ...
            success: function (msg) {
                ...
                if (msg == 'uspeh') {
                    $('#logoglas').hide();
                    $("#greska").html('Uspešno ste se registrovali!</br><button class="btn btn-theme " style="border:2px solid white; color:white; background-color:#3333CC" type="button" id="postavi">Postavi oglas</button>');
                    novi='1';
                } else {
                    $("#greska").html(msg);
                    ferror = true;
    ...
    $("body").on("click","#postavi", function() {
        ...
        if ($('#chbIzvorniText').prop("checked") == false) {
            ferror = true;
            $('#chbIzvorniText').parent().find('div').html('Neophodan podatak');
        } else {
        ...
        if ($('#chbUsloviKor').prop("checked") == false) {
            ferror = true;
            $('#chbUsloviKor').parent().find('div').html('Neophodan podatak');
        } else {
        ...
        if ($('#chbGarancija').prop("checked") == false) {
            ferror = true;
            $('#chbGarancija').parent().find('.validation').html('Neophodan podatak');
        } else {
        ...
        if ($("input[name='radioTrajanje']:checked").val() == undefined) {
            ferror = true;
            $('#sss').parent().find('div').html('Neophodan podatak');
        } else {
            $('#sss').parent().find('div').html('');
        }
        if ($("input[name='radioNacinOgl']:checked").val() == undefined) {
            ferror = true;
            $('#ncog').parent().find('div').html('Neophodan podatak');
        } else {
        ...
        if (!jmbgexp.test(a)) {
            ferror = true;
            $('#tbJmbg').parent().find('div').html('Upišite tačan JMBG');
        ...
        if (!telexp.test(b)) {
            ferror = true;
            $('#tbKontakt').parent().find('div').html('Upišite kontakt telefon');
        ...
        if (!telexp.test(t)) {
            ferror = true;
            $('#tbTelefon').parent().find('div').html('Upišite kontakt telefon');
        ...
        if (!pibexp.test(c)) {
            ferror = true;
            $('#tbPib').parent().find('div').html('Neophodno je upisati PIB firme');
        ...
        if (!pibexp.test(w)) {
            ferror = true;
            $('#tbMaticniBr').parent().find('div').html('Neophodno je upisati matični broj firme');
        ...
        if (ferror != true) {
            ...
            $.ajax({
                ...
                success: function (msg) {
                   ...
                   window.location.href = "uspeh.php?verifikacija";
               }
               if (msg == 'uspeh' && novi=='0'){
           window.location.href = "uspeh.php";
           }
                    else {
                        $("#greska").html(msg);
                        ferror = true;
            ...
            $("#greska").html('<Strong style="color: red;">Pogrešno popunjeni podaci!</Strong></br></br><button class="btn btn-theme " style="border:2px solid white;" type="button" id="postavi">Postavi oglas</button>');
        }
